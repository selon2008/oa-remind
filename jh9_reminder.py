...
# 课后延时服务课程(表格底部手写体，周一~周五)
DELAY = ["英语", "物理", "数学", "化学", "语文"]
...
def build_message(target, mode):
    ...
    lines.append(f"- 课后延时服务({T_DELAY}) {DELAY[wd]}")
    lines.append("小结:上午4节+大课间,下午3节,共7节;课后延时服务至18:30。")
    return "\n".join(lines)
